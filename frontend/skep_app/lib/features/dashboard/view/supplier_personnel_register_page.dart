...
/// 공급사 인력 등록 페이지 (단일 스크롤 페이지)
class SupplierPersonnelRegisterPage extends StatefulWidget {
  const SupplierPersonnelRegisterPage({Key? key}) : super(key: key);

  @override
  State<SupplierPersonnelRegisterPage> createState() =>
      _SupplierPersonnelRegisterPageState();
}

class _SupplierPersonnelRegisterPageState
    extends State<SupplierPersonnelRegisterPage> {
  final DocumentTypeRepository _docRepo = DocumentTypeRepository.instance;
  final PersonnelTypeRepository _pRepo = PersonnelTypeRepository.instance;
  ...
  // 서류 업로드 상태
  final Map<String, UploadedPersonnelDocument> _uploadedDocs = {};
  ...
  void _onPersonnelTypeChanged(String? type) {
    setState(() {
      ...
      _birthDate = null;
      if (type != null) {
        final requiredDocs = _pRepo.getRequiredDocuments(type);
        for (final docName in requiredDocs) {
          _uploadedDocs[docName] = UploadedPersonnelDocument();
  ...
  Future<void> _pickAndUploadFile(String docName) async {
    ...
    // 첫번째 서류면 OCR 실행
    final requiredDocs = _selectedPersonnelType != null
        ? _pRepo.getRequiredDocuments(_selectedPersonnelType!)
        : <String>[];
  ...
  void _parseOcrText(String text) {
    ...
    for (final line in lines) {
      ...
      if (name.isEmpty) {
        ...
        // "성명" 또는 "이름" 뒤에 오는 이름
        if (trimmed.contains('성명') || trimmed.contains('이름')) {
          final parts = trimmed.split(RegExp(r'[:\s]+'));
          for (final part in parts) {
            if (part != '성명' &&
                part != '이름' &&
                nameRegex.hasMatch(part) &&
                part.length >= 2 &&
                part.length <= 4) {
              name = part;
              break;
      ...
      // 생년월일 추출: "1990.01.01" 또는 "1990-01-01" 또는 "900101"
      if (birthDate.isEmpty) {
        final birthRegex1 =
            RegExp(r'(19|20)\d{2}[.\-/]\d{1,2}[.\-/]\d{1,2}');
        final birthMatch1 = birthRegex1.firstMatch(trimmed);
        if (birthMatch1 != null) {
          birthDate = birthMatch1.group(0) ?? '';
        }

        // 주민등록번호 앞자리에서 생년월일 추출
        if (birthDate.isEmpty) {
          final rrnRegex = RegExp(r'(\d{6})\s*[-]\s*\d');
          final rrnMatch = rrnRegex.firstMatch(trimmed);
          if (rrnMatch != null) {
            final rrn = rrnMatch.group(1) ?? '';
            if (rrn.length == 6) {
              final yy = int.tryParse(rrn.substring(0, 2)) ?? 0;
              final mm = rrn.substring(2, 4);
              final dd = rrn.substring(4, 6);
    ...
    setState(() {
      ...
      if (birthDate.isNotEmpty && _birthDateController.text.isEmpty) {
  ...
  bool _canRegister() {
    return _selectedPersonnelType != null &&
  ...
  List<String> get _missingDocuments {
    if (_selectedPersonnelType == null) return [];
    final requiredDocs = _pRepo.getRequiredDocuments(_selectedPersonnelType!);
  ...
  Future<void> _completeRegistration() async {
    if (!_canRegister()) {
      ...
      if (_nameController.text.trim().isEmpty) errors.add('이름을 입력해주세요');
      if (_phoneController.text.trim().isEmpty) errors.add('연락처를 입력해주세요');
  ...
  void _showImagePreview(UploadedPersonnelDocument doc) {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          // 헤더
          Text('인력 등록', style: AppTextStyles.headlineLarge),
          const SizedBox(height: 4),
          Text(
            '인력 유형을 선택하고, 서류를 업로드한 후 등록하세요.',
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('인력 유형 선택', style: AppTextStyles.headlineSmall),
                const SizedBox(height: 16),
                DropdownButtonFormField<String>(
                  value: _selectedPersonnelType,
                  decoration: const InputDecoration(
                    labelText: '인력 유형 *',
                    hintText: '인력 유형을 선택하세요',
                    border: OutlineInputBorder(),
                    prefixIcon: Icon(Icons.badge_outlined),
                  ),
                  items: _pRepo.typeNames
                      .map((type) =>
                          DropdownMenuItem(value: type, child: Text(type)))
                      .toList(),
                  onChanged: _onPersonnelTypeChanged,
                ),
              ],
            ),
          ),
          ...
          // 2. 서류 업로드 카드 목록
          if (_selectedPersonnelType != null) ...[
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    '$_selectedPersonnelType 유형의 필수 서류 ${requiredDocs.length}개를 업로드해 주세요. 첫번째 서류 업로드 시 OCR로 이름/생년월일이 자동 입력됩니다.',
  ...
  Widget _buildUploadedFileInfo(
      String docName, UploadedPersonnelDocument doc) {
    final isImage = _isImageFile(doc.fileName ?? '');
  ...
  Widget _buildVerificationStatus(UploadedPersonnelDocument doc) {
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (status == PersonnelVerificationStatus.verifying)
                Padding(
  ...
  String _verificationLabel(PersonnelVerificationStatus status) {
    switch (status) {
  ...
  Color _verificationColor(PersonnelVerificationStatus status) {
    switch (status) {
      case PersonnelVerificationStatus.pending:
